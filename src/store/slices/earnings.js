...
const getEarnings = createAsyncThunk(
  `${name}/getEarnings`,
  (payload, thunkAPI) => {
...
const createEarning = createAsyncThunk(`${name}/createEarnings`, (payload) => {
...
const updateEarning = createAsyncThunk(`${name}/updateEarning`, (payload) => {
  const earning = new EarningModel(payload);
...
const deleteEarning = createAsyncThunk(`${name}/deleteEarning`, (payload) => {
  const id = payload;
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteEarning.fulfilled, (state, action) => {
      const updatedItems = state.items.filter(
        (earning) => earning.id !== action.payload
      );
