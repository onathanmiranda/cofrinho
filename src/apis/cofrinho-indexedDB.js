        ...
        collections.forEach((collection) => {
          ...
          if (collection.indexes)
            collection.indexes.forEach((index) =>
              store.createIndex(`${index}_index`, index)
            );
        });
      },
      blocked: () => {
          ...
          database
            ...
            .then((expenses) =>
              database
                ...
                .then((earnings) =>
                  database.getAll("accounts").then((accounts) => {
                    const accountsLeftOvers = accounts.map((account) => {
                      const { quota, id } = account;
                      const totalEarned = earnings.reduce(
                        (acc, earning) => earning.amount + acc,
                        0
                      );
                      const accountTotal = totalEarned * quota;
                      const expensesOnAccount = expenses.filter(
                        (expense) => id === expense.account
                      );
                      const totalSpent = expensesOnAccount.reduce(
    ...
    collections.forEach((collection) => {
      ...
      this[collection.name].getAllKeys = () => {
        return this.database.then((database) =>
          database.getAllKeys(collection.name)
        );
      };
      ...
      this[collection.name].getAll = () => {
        return this.database.then((database) =>
          database.getAll(collection.name)
        );
      };
      ...
      this[collection.name].post = (value) => {
        return this.database.then((database) =>
          database.add(collection.name, value)
        );
      };
      ...
      this[collection.name].put = (value) => {
        return this.database.then((database) =>
          database
            .put(collection.name, value)
            .then((id) => database.get(collection.name, id))
        );
      };
      ...
      this[collection.name].delete = (value) => {
        return this.database.then((database) =>
          database.delete(collection.name, value)
        );
      };
      ...
      if (collection.indexes) {
        collection.indexes.forEach((index) => {
          ...
          this[collection.name][`getAllBy${capitalizedIndex}Range`] = (
            smaller,
            bigger
          ) => {
            return this.database.then((database) => {
          ...
          this[collection.name][`getAllBy${capitalizedIndex}LowerBound`] = (
            value
          ) => {
            return this.database.then((database) => {
          ...
          this[collection.name][`getAllBy${capitalizedIndex}UpperBound`] = (
            value
          ) => {
            return this.database.then((database) => {
        ...
        this.database.then((database) => {
          ...
          data.dump.forEach((store) => {
            store.data.forEach((entry) => {
              ...
              promises.push(database.add(store.storeName, entry));
            });
          });
