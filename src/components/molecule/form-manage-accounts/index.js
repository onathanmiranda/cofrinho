...
export default function FormManageAccounts({ onCancel, onSubmit }){
  ...
  let quotasSum = accounts.reduce((acc, { quota }) => {
    acc   = new Decimal( acc )
  ...
  const quotasSumErrorClassName = isQuotasSumDifferentFrom100 ? styles.error : ''
  ...
  function handleSubmit(e){
    ...
    Promise.all(accounts.map((account) => dispatchEvent(updateAccount(account))))
    .then(() => {
      ...
      {accounts.map(( account ) => {
        return (
          <div className={styles.account} key={account.id}>
            <label className={styles.accountTitle} htmlFor={account.title}>{account.title}</label>
            <PercentageInput name={account.title} onChange={(value) => handleOnChange(account.id, value)} value={account.quota} />
          </div>
        )
      })}
      <div className={`${styles.quotasSum} ${quotasSumErrorClassName}`}>
        <p>Total {quotasSum}%</p>
        {isQuotasSumDifferentFrom100 && <p className={styles.errorMessage}>A soma das porcentagens deve ser igual à 100%</p>}
      </div>
      <Button disabled={isQuotasSumDifferentFrom100} className={styles.submitButton} >Salvar</Button>
