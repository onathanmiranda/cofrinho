...
export default function FormEarning({
  onSubmit,
  onCancel,
  className = "",
  earningID,
  ...
  const earning = isEditing
    ? earnings.items.find(({ id }) => id === earningID)
    : new EarningModel({});
  ...
  const currentMonthTimestamp = useSelector(
    ({ timeline }) => timeline.current.timestamp
  );
