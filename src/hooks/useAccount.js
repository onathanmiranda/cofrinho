...
export default function useAccount(id){
  ...
  if(!id) throw new Error(`You must provide an account ID{int} to useAccount Hook. Provided ${id} instead.`);
  ...
  const expenses      = useSelector(({ expenses })  => expenses.items.filter(( expense ) => expense.account === id ))
